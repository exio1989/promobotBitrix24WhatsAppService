...
//		$url = 'https://'.self::OAUTH_SERVER.'/oauth/token/'.
    ...
    {
        if ('' === $webhookurl) {
            throw new Bitrix24Exception('webhookurl is empty');
        }
...
//		$url = 'https://'.self::OAUTH_SERVER.'/rest/app.info?auth='.$accessToken;
	...
	public function addSimpleBatchCall($query)
    {
        $id = uniqid();
        $this->_batch[$id] = $query;
        return $id; 
    } 
	
	public function processSimpleBatchCalls($halt = 0, $delay = self::BATCH_DELAY)
    {
		$results = array();
		$results["result"] = array();
		$results["result_total"] = array();
		$results["result_next"] = array();
		...
		while (count($this->_batch)) {
			...
			$batchResult = $this->call('batch', array('halt' => $halt, 'cmd' => $commands));
			$resultsPart = $batchResult['result'];
			
			foreach($resultsPart["result"] as $k=>$v){
				$results["result"][$k] = $v;
			}
			
			foreach($resultsPart["result_total"] as $k=>$v){
				$results["result_total"][$k] = $v;
			}
			
			foreach($resultsPart["result_next"] as $k=>$v){
				$results["result_next"][$k] = $v;
			}  
			if (count($this->_batch) && $delay) {
                usleep($delay);
            }
    ...
    public function call($methodName, array $additionalParameters = array())
    {
		...
		while($retryCounter<$maxRetries){
			try {
				...
				if (!$retry) {
					$this->log->error("not retry");
					throw $e;
				}
				$this->log->warning("try ".$retryCounter." failed");
				$retryCounter++;
			}
		}        
    }
    ...
    protected function _call($methodName, array $additionalParameters = array())
    {
		if (null === $this->getWebhookUrl()) {
			...
			$url = $this->getWebhookUrl() . $methodName;
		}
